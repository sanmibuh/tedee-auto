...
@RestClientTest(LockLockedHandler.class)
@Import(TedeeClientConfiguration.class)
@TestPropertySource(
    properties = {
      "sanmibuh.rest.tedee.base-url=" + LockLockedHandlerTest.BASE_URL,
      "sanmibuh.rest.tedee.api-key=" + LockLockedHandlerTest.API_KEY
    })
class LockLockedHandlerTest {
  ...
  @Autowired private LockLockedHandler sut;
  ...
  @Test
  void should_postToLockEndpoint_whenHandlingLockLocked() {
    server
        .expect(requestTo(LOCK_URL))
        .andExpect(method(HttpMethod.POST))
        .andRespond(withNoContent());

    sut.handle(new LockLocked(DEVICE_ID));

    server.verify();
  }

  @Test
  void should_sendApiToken_whenHandlingLockLocked() {
  ...
  void should_translateBridgeError_whenBridgeRespondsWithError(
    ...
    thenThrownBy(() -> sut.handle(new LockLocked(DEVICE_ID))).isInstanceOf(expectedException);
  }
  ...
  void should_throwLockTemporarilyUnavailableException_whenBridgeIsUnreachable() {
    ...
    thenThrownBy(() -> sut.handle(new LockLocked(DEVICE_ID)))
