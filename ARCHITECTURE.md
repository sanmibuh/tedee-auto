...
6. **Commit, tag and push the release** to `main`: one `Release vX.Y.Z` commit (pom + `CHANGELOG.md`), pushed with a rebase-and-retry loop that **fails the job if every attempt fails** (so a tag or release can never be created without the commit on `main`). The `vX.Y.Z` tag is created **only after** the push succeeds, so it points at the exact commit that landed on `main` even if a concurrent push forced a rebase.
7. **Bump to next development snapshot** — a second commit `Prepare development iteration vX.Y.(Z+1)-SNAPSHOT` returns `main` to a development version. This runs **before** creating the GitHub Release so that both commits are on `main` (and `main` is back on `-SNAPSHOT`) before the only remaining fallible step.
8. **Create the GitHub Release** from the tag using the frozen release notes. This runs last on purpose: it is the only externally-visible step that cannot leave `main` inconsistent, since `main` already carries both commits. It is idempotent (skips if the release already exists) and retried, and can always be recreated by hand from the existing tag.

The net effect is exactly two commits on `main` — the tagged release and the snapshot bump — and only on full success. On any failure before step 6, `main` is left untouched on its `-SNAPSHOT` version, with no tag and no release.

The changelog rendering (category matching and Markdown formatting) lives in `.github/scripts/render_changelog.py`. The rebase-and-retry push to `main`, shared by both release commits, lives in `.github/scripts/push-main-with-retry.sh`. The changelog query uses `gh api --paginate` so a release window with more than 100 updated PRs is never silently truncated.
