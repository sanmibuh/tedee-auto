...
7. **Create the GitHub Release** from the tag using the frozen release notes. This runs **before** the `latest` alias update so a transient GHCR failure on that convenience alias can never skip release creation. It is idempotent (skips if the release already exists) and retried, and can always be recreated by hand from the existing tag.
8. **Advance the `latest` alias** to this build, only now that `main`, the tag and the GitHub Release are all published — so an aborted run never moves `latest` onto an unreleased image. It is retried on transient GHCR errors; a persistent failure leaves the immutable `:version` image, tag and release intact, and `latest` can be repointed by hand.
