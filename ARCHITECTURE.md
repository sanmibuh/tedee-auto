...
6. **Commit, tag and push the release** to `main`: one `Release vX.Y.Z` commit (pom + `CHANGELOG.md`) and the `vX.Y.Z` tag. Pushes use a rebase-and-retry loop to tolerate a concurrent push to `main`.
7. **Create the GitHub Release** from the tag using the frozen release notes.
8. **Bump to next development snapshot** — a second commit `Prepare development iteration vX.Y.(Z+1)-SNAPSHOT` returns `main` to a development version.
...
The changelog rendering (category matching and Markdown formatting) lives in `.github/scripts/render_changelog.py`.
